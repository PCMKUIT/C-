...
class QuanLyDanhSach{
	...
	public:
	QuanLyDanhSach(){DuLieu.resize(0);}
	~QuanLyDanhSach(){
		if(DuLieu.size() > 0){
	...
	void Nhap(){
		...
		while (true){
			...
			cout<<"\n4.Nghe si";
			cout<<"\n5.Ca si";
			cout<<"\n6.Chot danh sach va xuat ra danh sach thong ke";
			cout<<"\nNhap mot so nguyen tu 1 den 6: ";
			cin>>LoaiDoiTuong;
			while(LoaiDoiTuong<1||LoaiDoiTuong>6)
				{cout<<"Cau lenh khong hop le, vui long nhap lai mot so nguyen tu 1 den 6: "; cin>>LoaiDoiTuong;}
			switch (LoaiDoiTuong){
				case 1: DuLieu.pb(new SinhVien); break;
				case 2: DuLieu.pb(new HocSinh); break;
				case 3: DuLieu.pb(new CongNhan); break;
				case 4: DuLieu.pb(new NgheSi); break;
				case 5: DuLieu.pb(new CaSi); break;
				case 6: close = 1; break;}
			if(close) break;
			DuLieu.back()->Nhap(); cout<<endl;}}
