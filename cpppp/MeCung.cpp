	...
	while(true){
		...
		if(LuaChon==Road) {
			...
			if(Percent>0&&Percent<8&&!Note[0]){
				...
				Boost=3; Note[0]=true;}
			else if(Percent>7&&Percent<18&&!Note[0]&&!Note[1]){
				if(Boost==1) {
				cout<<"\nDang chay giua chung ban lai thay 1 doi giay moi , ban lien coi doi giay hien tai ra";
				...
				Boost=2; Note[1]=true;}
			else if(Percent>17&&Percent<31&&!Note[0]&&!Note[1]&&!Note[2]){
			...
			Fixed=0;}}
	cout<<"\nChang duong hoan thanh: "<<Round-1<<"/"<<x<<".";
	return 0;}